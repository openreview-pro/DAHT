...
class TransformerLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_heads: int, use_bn: bool = False, drop_rate: float = 0.5, is_last: bool = False):
        ...
        self.head_dim = out_channels // num_heads  # Define head_dim

    def forward(self, X: torch.Tensor, hg: Hypergraph) -> torch.Tensor:
        # Compute node features (multi-head attention)
        Q = self.query(X).view(-1, self.num_heads, self.head_dim)  # Shape: [N, num_heads, head_dim]
        K = self.key(X).view(-1, self.num_heads, self.head_dim)    # Shape: [N, num_heads, head_dim]
        V = self.value(X).view(-1, self.num_heads, self.head_dim)  # Shape: [N, num_heads, head_dim]
        ...
        attention_nodes = F.softmax(torch.matmul(Q, K.transpose(-1, -2)) / math.sqrt(self.head_dim), dim=-1)
        X_node = torch.matmul(attention_nodes, V).view(-1, self.num_heads * self.head_dim)  # Shape: [N, out_channels]

        # Compute attention between hyperedges and nodes (single-head attention)
        attention_hyperedges = F.softmax(torch.matmul(Qe, Ke.transpose(-1, -2)) / math.sqrt(self.head_dim), dim=-1)  # Shape: [N, E]
        ...
        #
        # # Compute attention between hyperedges and nodes (single-head attention)
        # attention_hyperedges = F.softmax(torch.matmul(Qe, Ke.transpose(-1, -2)) / math.sqrt(Q.shape[-1]),
        #                                  dim=-1)  # Shape: [N, E]
        # X_hyperedge = torch.matmul(attention_hyperedges, Xe)  # Shape: [N, out_channels]

        # Combine the results from node-node and hyperedge-node attention
        X = X_hyperedge + X_node
        # X = X_hyperedge
        X = X_node
